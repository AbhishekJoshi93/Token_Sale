...
contract('AbhiToken', function (accounts) {
    ...
    it('initializes the contract with correct value', function () {
        return AbhiToken.deployed().then(function (instance) {
            ...
            assert.equal(name, 'Abhi Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function (symbol) {
            assert.equal(symbol, 'Abhi', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function (standard) {
            assert.equal(standard, 'Abhi Token v1.0', 'has the correct standard');
        })
    });
